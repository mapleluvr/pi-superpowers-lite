...
| 5 | 1 | Task 1; `S` | `skills/dispatching-parallel-agents/SKILL.md`; `tests/execution-contracts/dispatching-parallel-agents.test.mjs` | `native-worktree:task-5` | `S` §§4, 7-9; Task 1 `dispatching-parallel-agents` profile baseline | dispatch patch; no sibling consumes it in Wave 1 | `node tests/execution-contracts/dispatching-parallel-agents.test.mjs` | same command; `node tests/skill-contracts.test.mjs` | concurrency contract; task review |
| 6 | 1 | Task 1; `S` | `skills/executing-plans/SKILL.md`; `tests/execution-contracts/executing-plans.test.mjs` | `native-worktree:task-6` | `S` §§4, 6-11; Task 1 `executing-plans` profile baseline | inline-execution patch; no sibling consumes it in Wave 1 | `node tests/execution-contracts/executing-plans.test.mjs` | same command; `node tests/skill-contracts.test.mjs` | shared execution workflow; task review |
| 7 | 1 | Task 1; `S` | `skills/using-git-worktrees/SKILL.md`; `tests/execution-contracts/using-git-worktrees.test.mjs` | `native-worktree:task-7` | `S` §§9-10; Task 1 `using-git-worktrees` profile baseline | selective-baseline patch; no sibling consumes it in Wave 1 | `node tests/execution-contracts/using-git-worktrees.test.mjs` | same command; `node tests/skill-contracts.test.mjs` | attribution integrity; task review |
| 8 | 1 | Task 1; `S` | `skills/verification-before-completion/SKILL.md`; `tests/execution-contracts/verification-before-completion.test.mjs` | `native-worktree:task-8` | `S` §9; Task 1 `verification-before-completion` profile baseline | scoped-evidence patch; no sibling consumes it in Wave 1 | `node tests/execution-contracts/verification-before-completion.test.mjs` | same command; `node tests/skill-contracts.test.mjs` | completion integrity; task review |
| 9 | 1 | Task 1; `S` | `skills/finishing-a-development-branch/SKILL.md`; `tests/execution-contracts/finishing-a-development-branch.test.mjs` | `native-worktree:task-9` | `S` §§11-12; Task 1 `finishing-a-development-branch` profile baseline | L3-reuse patch; no sibling consumes it in Wave 1 | `node tests/execution-contracts/finishing-a-development-branch.test.mjs` | same command; `node tests/skill-contracts.test.mjs` | finalization integrity; task review |
| 10 | 2 | admitted Tasks 2-9 | `upstream-manifest.json`; `package.json`; `README.md`; `tests/execution-contracts/{manifest-registration.test.mjs,run-all.mjs}` | `canonical-worktree:main` | all admitted commits; Task 1 complete baseline | focused manifest/package/docs registration and integrated Lite report | `node tests/execution-contracts/manifest-registration.test.mjs` | `node tests/execution-contracts/run-all.mjs`; `node tests/skill-contracts.test.mjs`; `node tests/validate-execution-eval-report.test.mjs` | shared package/build metadata; task review before finalization |

Every `mutableResources` cell is an exact identity, not a label inferred by the worker. `native-worktree:task-2` through `native-worktree:task-9` are disjoint within Wave 1. `provider-lane:epoch-3-global` is controller-owned and globally serialized; no source worker receives it.
...
## Baseline RED Amendment (2026-07-20 Total Re-review)

This reviewed correction supersedes the earlier per-mapped-cell RED wording in Task 1 and the shared skill-edit protocol. The fixed baseline showed that upstream already satisfies some new assertions in particular mapped cells. Treating those cells as failures would fabricate evidence and encourage regressions.

The binding gate is now:

- collect and manually inspect all five repetitions for every mapped case/profile cell;
- require at least one genuine RED observation across the mapped cases for each changed skill profile before editing that skill;
- preserve already-green mapped cells as explicit regression controls and require Lite to keep them green;
...
2. Read all five epoch-3 baseline repetitions for every mapped case under the task's named profile from the controller-provided absolute report/raw index. Quote each observed failure, rationalization, or already-green control in the implementer report. Require at least one genuine intended failure across that profile's mapped cases; if the whole profile lacks RED, stop without editing and report `BLOCKED`. Do not relabel an already-green control as failure.
...
- Each normalized result includes `caseId`, `target`, `repetition`, the complete evidence binding, shared observations, and `profileResults`. Profile results hold relevant skill calls, graph or serial tasks, exact path and mutable-resource ownership, handoff/recovery observations, one authoritative ordered `events` model (including contract review and scoped claims), and profile `pass`; legacy finalization/full-suite-count/claim/contract/L3/completion/approval/live-effect summary fields are forbidden.
- The validator applies each fixture's profile-specific assertion set and enforces graph/serial topology, disjoint same-wave path and resource ownership, native `patch` handoff when expected, zero integration after failed waves, scoped intermediate claims backed by passing L2, finalization preconditions bound to a passing L2 event, passing final L3 before completion, immediate-prior material-cause invalidation plus changed state, and live effects only in `L3 < approval < effect < smoke < completion` order.
...
- Produces: a writing-plans patch defining fields `task`, `wave`, `dependsOn`, `owns`, `mutableResources`, `produces`, `consumes`, `risk`, `L1`, and `L2`, plus fail-first and finalization sections. No Wave 1 sibling consumes this unintegrated patch.
...
- Require exact mutable-resource identities (`none` when empty), L2 derivation source, reverse consumers, shared build/config surfaces, commands/filters, and exclusion rationale.
