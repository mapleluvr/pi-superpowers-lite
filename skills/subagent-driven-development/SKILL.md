...
Then dispatch one mandatory final whole-branch review from the recorded branch start through `HEAD`, including authority, manifest/frontiers, commits, full diff, L1/L2/L3 evidence, known risk, and deferred live effects. The final gate has one initial pass, at most one consolidated correction frontier, and one closure re-review. The closure scope is accepted findings, fix diff, and regression evidence; unrelated non-Critical findings become deferred final-review risks. A confirmed Critical regression or false evidence may reopen the gate.
